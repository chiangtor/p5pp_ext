...
dnl PHP_ARG_ENABLE(myfile, whether to enable myfile support,
dnl Make sure that the comment is aligned:
dnl [  --enable-myfile           Enable myfile support])
...
if test "$PHP_MYFILE" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $MYFILE_DIR/lib, MYFILE_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_MYFILELIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong myfile lib version or lib not found])
  dnl ],[
  dnl   -L$MYFILE_DIR/lib -lm
  dnl ])
  dnl
